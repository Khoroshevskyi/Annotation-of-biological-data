...
class FindWikiPage(object):
    ...
    def get_item_descriptions(self, statement_id):
        try:
            item_dict = self.get_item_info(statement_id)
            ...
            logging.warning(f"{statement_id} No description were found")
            return ""
    ...
    def search_by_property(self, id_info, property_id, value):
        logging.error(f"Searching property value: {value} by property id: {property_id} in {id_info}")
        statement_id_list = self.search_statements(id_info, property_id)  # searching statement value by property id
    ...
    def try_different_configuration(self, text_list):
        ...
        text_list1[0:1] = text_list1[1:0:-1]
...
if __name__ == "__main__":
    ...
    print(f"Ids that were found: {id_found} ")
